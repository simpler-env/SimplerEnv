...
def load_annotations(root_path, uids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    object_paths = _load_object_paths(root_path)
    dir_ids = (
...
def _load_object_paths(root_path) -> Dict[str, str]:
...
def load_uids(root_path) -> List[str]:
    """Load the uids from the dataset.

    Returns:
        A list of uids.
    """
    return list(_load_object_paths(root_path).keys())
...
def load_objects(root_path: str, uids: List[str], download_processes: int = 1) -> Dict[str, str]:
    ...
    if download_processes == 1:
        ...
        for uid in uids:
            ...
            local_path = os.path.join(root_path, object_path)
            if not os.path.exists(local_path) or os.path.getsize(local_path) < 1024:
                args.append((root_path, uid, object_paths[uid]))
            else:
                out[uid] = local_path
        if len(args) == 0:
            return out
...
def load_lvis_annotations(root_path) -> Dict[str, List[str]]:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root-path', type=str, default='/home/xuanlin/objaverse')
    parser.add_argument('--uids', nargs='+', type=str, default=None)
    ...
    # ]
    # uids = ['2d80d079a75141f4a4a555aeffa1b9a4', 'c62d9577121048779abe2e66fbd17785'] # opened coke can
    
    annotations = load_annotations(args.root_path, uids)
    final_uids = []
    for k in uids:
    ...
    objects = load_objects(args.root_path, uids, download_processes=min(10, len(uids)))
