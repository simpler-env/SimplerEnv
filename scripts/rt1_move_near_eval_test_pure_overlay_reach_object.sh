export MS2_ASSET_DIR=/home/xuanlin/Real2Sim/ManiSkill2_real2sim/data

env_name=MoveNearGoogleInScene-v0
scene_name=google_pick_coke_can_1_v4
gpu_id=0

declare -a arr=("/home/xuanlin/Real2Sim/xid77467904_000400120/" \
                "/home/xuanlin/Real2Sim/rt1poor_xid77467904_000058240/" \
                "/home/xuanlin/Real2Sim/rt_1_x_tf_trained_for_002272480_step/")

for ckpt_path in "${arr[@]}"; do echo "$ckpt_path"; done

for ckpt_path in "${arr[@]}";

do for i in {1..5}; 
do for j in {1..2};


do rgb_overlay_path=/home/xuanlin/Real2Sim/ManiSkill2_real2sim/data/real_impainting/move_near_real_${i}_${j}.png;

let x=(${i}-1)*2;
let x=${x}+${j}-1;
let x=${x}*6;
let y=(${i}-1)*2;
let y=${y}+${j};
let y=${y}*6;
echo ${x} ${y};

CUDA_VISIBLE_DEVICES=${gpu_id} python real2sim/main_inference.py --policy-model rt1 --ckpt-path ${ckpt_path} \
  --robot google_robot_static --additional-env-save-tags test_pure_overlay_reach_object \
  --control-freq 3 --sim-freq 513 --max-episode-steps 80 \
  --env-name ${env_name} --scene-name ${scene_name} \
  --rgb-overlay-path ${rgb_overlay_path} \
  --robot-init-x 10.35 10.35 1 --robot-init-y 10.21 10.21 1 --obj-variation-mode episode --obj-episode-range ${x} ${y} \
  --robot-init-rot-quat-center 0 0 0 1 --robot-init-rot-rpy-range 0 0 1 0 0 1 -0.09 -0.09 1;

done
done
done





